...
def model_fn(model_dir): #retreive trained model data 
    print("Loading model.")
...
def train(model, train_loader, epochs, optimizer, criterion, device):
    ...
    for epoch in range(1, epochs + 1):
        model.train()
        total_loss = 0
        for batch_idx, (data, target) in enumerate(train_loader, 1):  #1 is the start index 
            # prep data
...
#this code will be executed when .fit is called 
if __name__ == '__main__':
    ...
    parser.add_argument('--model-dir', type=str, default=os.environ['SM_MODEL_DIR']) #where to access the trained model information
    parser.add_argument('--data-dir', type=str, default=os.environ['SM_CHANNEL_TRAIN']) #where to access the training data set 
    ...
    criterion = nn.BCELoss() #used in classification  
